...
export function createScreen(canvas) {
  ...
  // Equivalent to CoCo CLS [color]. We render a uniform background by filling vram with
  // an all-quadrants semigraphic of the requested color (or 32 = blank for color 0).
  // colorIndex 0 = black background, 1-8 = colored background per CoCo CLS argument.
  function cls(colorIndex = 0) {
    const code = colorIndex === 0 ? 32 : 128 + (((colorIndex - 1) & 0x7) << 4) + 15
    vram.fill(code)
    inverseFlags.fill(0)
    redrawAll()
  }
...
test('tempo, octave, default length, volume tokens are state changes', () => {
  ...
  assertEquals(events[0], { type: 'tempo', value: 120 })
  assertEquals(events[1], { type: 'octave', value: 3 })
  assertEquals(events[2], { type: 'length', value: 8 })
  assertEquals(events[3], { type: 'volume', value: 20 })
...
export function createAudio() {
  ...
  return { play, stop, resume, suspend }
}
```
...
async function playRounds(screen, audio, input) {
  ...
  let pausedTicks = 0  // ticks accumulated by previous descents (TIMER reset per run in original)
  ...
  while (true) {
    ...
    if (result.kind === 'reachedBottom') {
      runs += 1
      // BASIC line 380-400: HT=TIMER (kept), brief celebration, reset for next run unless Q=3.
      if (runs >= 3) {
        return ticksElapsed(startMs)
      }
      await celebrateRun(screen, audio)
    }
  }
}

function ticksElapsed(startMs) {
  return Math.floor((performance.now() - startMs) / (1000 / 60))
}
